...
st.markdown(
    ...
    .app-title { font-size: 35px; font-weight: 900; color: white; margin: 0; }
    ...
    .subtle-text { color: #b8c6e3; font-size: 13px; }
    </style>
    """,
...
def init_files():
    ...
    if not INGRESSOS_PATH.exists():
        ...
        ]).to_csv(INGRESSOS_PATH, index=False)
...
def menu():
    return option_menu(
        ...
        styles={
            "container": {"padding": "0!important", "background-color": "rgba(255,255,255,0.03)", "border-radius": "16px", "border": "1px solid rgba(255,255,255,0.08)"},
...
def page_dashboard():
    ...
    with info1:
        st.markdown('<div class="glass-card"><strong>Identidade visual aplicada</strong><br><span class="subtle-text">A logo oficial do clube e a arte temática do Arraiá já estão integradas ao layout do sistema.</span></div>', unsafe_allow_html=True)
    with info2:
        st.markdown(f'<div class="glass-card"><strong>Usuário atual</strong><br><span class="subtle-text">{st.session_state.current_user}</span></div>', unsafe_allow_html=True)
